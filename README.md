...
Local-first musical intelligence. You provide taste and intention. Session Agents provide craft. PMR holds musical meaning. Hear sketches in Poppin, then optionally open them in GarageBand (or Reaper).

Phase 1: **New Session → Generate → Audition (Play) → Send to GarageBand**. Reaper send remains available but is optional. The composition canvas is not built yet.
...
4. Press **Play** on Audition — hear harmony / bass / drums in the browser (no DAW).
...
6. Regenerate if weak. Keep if good. **Reaper (optional)** still writes a `.rpp` if you want it.
...
packages/daw-bridge  GarageBand MIDI write / optional Reaper bridge
